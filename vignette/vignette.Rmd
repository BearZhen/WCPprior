...
The following code performs the approximation.
```{r}
# the Wasserstein distance function, where x[1] is the mean parameter, x[2] is the standard deviation parameter.
W_func <- function(x){sqrt(x[1]^2+x[2]^2)}
# user-specified hyperparameter
eta = 1
# cutoff parameter
cutoff = 0.01
# region information, type of region is conic, range of the angle of the conic region is from 0 to pi and the base model parameter is (0,0)
region = list(type = 'conic', lower_angle = 0, upper_angle = pi, base_theta = c(0,0))
# mesh width
mesh_width = 0.5

# call the function to compute numerical approximation of the WCP prior.
# By setting parallel = TRUE, user can run the function with multiple cores and NumCores specifies number of cores.
# By setting visual_mesh = TRUE, 
...
Now we show the approximated density.
```{r}
# extract (theta1, theta2) where the approximated density defines on
density_location = result[[1]]
# extract approximated density
approx_WCP_density = result[[2]]

data = cbind(as.vector(density_location[,1]),as.vector(density_location[,2]),as.vector(approx_WCP_density))
